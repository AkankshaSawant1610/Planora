...
public class SubmitBookingServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (eventId == null || eventTitle == null || eventDate == null || name == null || email == null || ticketCountStr == null) {
            response.sendRedirect("book_ticket.jsp?eventId=" + eventId + "&error=missing_fields");
            return;
        }
        ...
        try {
            ...
            response.sendRedirect("book_ticket.jsp?eventId=" + eventId + "&error=invalid_count");
            return;
        }
        ...
        if (ticketPrice == 0) {
            response.sendRedirect("book_ticket.jsp?eventId=" + eventId + "&error=invalid_event");
            return;
        }
        ...
        try (
            ...
            if (rows > 0) {
                ...
                RequestDispatcher dispatcher = request.getRequestDispatcher("confirm_booking.jsp");
                dispatcher.forward(request, response);
            } else {
                response.sendRedirect("book_ticket.jsp?eventId=" + eventId + "&error=insert_fail");
            }

        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect("book_ticket.jsp?eventId=" + eventId + "&error=exception");
        }
    }
}
